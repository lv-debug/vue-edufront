    ...
    <section class="container">
      ...
      <div class="t-infor-wrap">
        ...
        <section class="fl t-infor-box c-desc-content">
          <div class="mt20 ml20">
            ...
            <h3 class="hLh30">
              <span class="fsize24 c-333">{{teacherData.name}}&nbsp;{{teacherData.level === 1?'高级讲师':'首席讲师'}}</span>
            </h3>
            <section class="mt10">
              <span class="t-tag-bg">{{teacherData.career}}</span>
            </section>
      ...
      <section class="mt30">
        <div>
          ...
          <!-- /无数据提示 开始-->
          <section class="no-data-wrap" v-if="courseData.length === 0">
            <em class="icon30 no-data-ico">&nbsp;</em>
            <span class="c-666 fsize14 ml10 vam">没有相关数据，小编正在努力整理中...</span>
          </section>
          <!-- /无数据提示 结束-->
          <article class="comm-course-list" v-if="courseData.length > 0">
            <ul class="of">
...
export default {
  ...
  asyncData({ params , error }) {
    //此处的id等价于_id,所以必须.id。
    return teacherApi.getTeacherCourseById(params.id).then(response =>{
        ...
        return {
          ...
          courseData : response.data.data.teacherCourse
        }
    })
  },
